...
def extract_item_info(item, additional_info={}):
    ...
    if primary_type != 'channel':
        ...
        info['author_id'] = extract_str(multi_deep_get(item,
            ...
            ['onTap', 'innertubeCommand', 'reelWatchEndpoint', 'playlistId'], # some shorts # mine
        ))
    ...
    if primary_type == 'video':
        ...
        info['approx_subscriber_count'] = extract_approx_int(multi_deep_get(item, ['videoCountText', 'simpleText'])) # mine
    ...
    if primary_type in ('playlist', 'channel'):
        ...
        conservative_update(info, 'video_count', None) # mine response does not contain number of videos
...
nested_renderer_list_dispatch = {
    ...
    'shelfRenderer': lambda r: (deep_get(r, 'content', 'horizontalListRenderer', 'items', default=()), None), # mine for topic playlist - need to test
}

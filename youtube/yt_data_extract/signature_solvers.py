...
def get_js_runtime(supported_runtimes):
    for runtime in supported_runtimes:
        js_runtime = find_executable(runtime)
        if js_runtime: return js_runtime
    return None

g_js_runtime = get_js_runtime([ 'node', 'deno', 'bun'])

def _run_js_runtime_bytes(js_file, requests, js_format="file"):
    if not g_js_runtime:
        raise NameError("No supported g_js_runtime is found!\nSupported runtimes: node.")
...
def _run_js_runtime_file(js_file, *args, js_format="file", response_format='json', response_type=None):
    ...
    with Popen(cmd, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE) as process:
        stdout, stderr = process.communicate_or_kill()
        output = None

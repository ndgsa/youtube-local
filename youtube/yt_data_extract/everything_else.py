...
def extract_channel_info(polymer_json, tab, continuation=False):
    ...
    if tab in ('videos', 'shorts', 'streams', 'playlists', 'search'):
        ...
        if items:
            ...
            info['date_joined'] = extract_date(deep_get(a_metadata, 'joinedDateText', 'content')) # mine
    ...
    # check_for_empty_value('extract_channel_info', info, ['error', 'items', 'links', 'avatar', 'ctoken']) # mine
...
def extract_playlist_metadata(polymer_json):
    ...
    # check_for_empty_value('extract_playlist_metadata', metadata, ['error', 'description', 'like_count', 'view_count', 'time_published']) # mine

...
database_path = os.path.join(settings.data_dir, 'db', "subscriptions.sqlite") # mine

def open_database():
    if not os.path.exists(os.path.join(settings.data_dir, 'db')): # mine
        os.makedirs(os.path.join(settings.data_dir, 'db')) # mine
...
def serve_subscription_thumbnail(thumbnail):
    ...
    if use_sqlite3_db_as_storage(): return serve_subscription_thumbnail_sqlite_db(thumbnail) # mine

...
_CHANNEL_AGGREGATE_PLAYLISTS_NAME_RE = re.compile(r'''^cha_\sU.+\s\-\s(playlists)$''')
...
def is_custom_type_playlist_name(name, custom=None):
    if custom == 'sa' and not _SEARCH_AGGREGATE_PLAYLIST_NAME_RE.match(name):
        ...
        is_custom = None
    elif custom == 'imported' and not _IMPORTED_PLAYLIST_VIDEOS_NAME_RE.match(name):
        is_custom = None
        ...
        is_custom = True
    elif _IMPORTED_PLAYLIST_VIDEOS_NAME_RE.match(name):
        is_custom = True
...
def sort_database_playlist(playlist_name, sort1='1', sort1_reversed=None, sorted_dublicate=True):
    ...
    if sorted_dublicate:
        if is_custom_type_playlist_name(playlist_name, 'sa'):
            ...
            sorted_playlist_name = re.sub(r"((videos|playlists))$", fr"sort1_{sort1} - \1", playlist_name)

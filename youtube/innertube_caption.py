...
def get_caption_json_resp(video_id, lang: str = 'en', auto_generated: bool = False):
    ...
    # ytcfg = util.INNERTUBE_CLIENTS['android']
    # working_ua = ytcfg['INNERTUBE_CONTEXT']['client']['userAgent'].replace(' gzip', '') #util.desktop_user_agent
    # visitor_data = util.get_visitor_data()
    # header = { 'User-Agent': working_ua, 'X-Goog-Visitor-Id': visitor_data }
    # payload = {}
    ...
    # except Exception as e:
        # print(f"Transcript is unavailable for video: {video_id} - {e}")
        # resp = None

    try:
        resp = util.call_youtube_api('android', 'get_transcript', {'params': params})
    except Exception as e:
    ...
    vtt_body = multi_deep_get(caption_data,
    ...
    ['actions', 0, 'elementsCommand', 'transformEntityCommand', 'arguments', 'transformTranscriptSegmentListArguments', 'overwrite', 'initialSegments'],
    default=None)
    ...
    if vtt_body:
        ...
        for item in continuation_submenu:
            if lang in item['title']:
                ...
                try:
                    resp_new = util.call_youtube_api('android', 'get_transcript', {'params': params_new})
                except Exception as e:

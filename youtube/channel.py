...
def get_number_of_videos_channel(channel_id):
    ...
    try:
        response = util.fetch_url(url, headers_desktop, # mine
    ...
    if match:
        ...
        return get_number_of_videos_channel_from_about_tab(channel_id) # mine
...
def get_channel_id(base_url):
    ...
    for channel_id_re in channel_id_re_list: # mine
        match = channel_id_re.search(response) # mine
        if match:
...
def get_metadata(channel_id, if_error=None): # mine
...
def get_channel_page_general_url(base_url, tab, request, channel_id=None):
    ...
    if tab in ('shorts', 'streams') or (tab=='videos' and try_channel_api):
        ...
        ctoken, continuation, number_of_pages = extract_ctoken(base_url, channel_id, tab, sort, page_number + 1, view, polymer_json=polymer_json) # mine
    ...
    if tab in ('videos', 'shorts', 'streams'):
        ...
        info['number_of_pages'] = number_of_pages if number_of_pages else math.ceil(number_of_videos/page_size) # mine
...
def get_path_of_keys(data_dict, key_name):
...
def extract_ctoken(base_url, channel_id, tab, sort, page_number, view, polymer_json=None):
    ...
    if not polymer_json:
        ...
        continuationCommand_list = get_path_of_keys(json.loads(polymer_json), 'chipCloudChipRenderer')
        if continuationCommand_list:
            # 0 - "Latest", 1 - "Popular", 2 - "Oldest" # this is the order when extracting from 'chipCloudChipRenderer' dict key
            if sort in ['1', '2']: ctoken = continuationCommand_list[int(sort)][1]['navigationEndpoint']['continuationCommand']['token']
            elif int(sort) > 2: ctoken = continuationCommand_list[0][1]['navigationEndpoint']['continuationCommand']['token']
        else:
            ctoken = multi_deep_get(json.loads(polymer_json),
        ...
        if not ctoken:
            continuationItemRenderer = get_path_of_keys(json.loads(polymer_json), 'continuationItemRenderer')
    ...
    # response, err = yt_data_extract.extract_response(json.loads(polymer_json))

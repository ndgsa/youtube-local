...
def generate_api_headers(ua_platform='innertube', client_name=None, use_visitor=False, additional_headers=(), update_headers_with=()):
    ...
    client_xhr_headers = {
        ...
        'X-Goog-Visitor-Id': get_visitor_data(client_name or client) if use_visitor else None,}

    if ua_platform == 'desktop':
        client_headers = {
            'X-YouTube-Client-Name': '1',
            'X-YouTube-Client-Version': INNERTUBE_CLIENTS['web']['INNERTUBE_CONTEXT']['client']['clientVersion'], # '2.20180830'
...
def call_youtube_api(client, api, data, query_params=None, use_visitor=True, report_text='', debug_name=None):
    ...
    if query_params: url = url + '?' + urllib.parse.urlencode(query_params)
    elif key and not query_params: url = url + '?key=' + key

    data = json.dumps(data)
    response = fetch_url(
        url, data=data, headers=headers,
        debug_name=debug_name if debug_name else 'youtubei_' + api + '_' + client,
        report_text=report_text if report_text else None if report_text is None else 'Fetched ' + client + ' youtubei ' + api
    ).decode('utf-8')
    return response
...
def get_visitor_data_from_homepage():
    ...
    yt_resp = fetch_url(
        ...
        headers={'User-Agent': get_user_agent('mobile')},
...
def get_visitor_data_(client, visitor_type):
    ...
    if not os.path.isfile(visitor_data_cache):
        visitor_data_dict = generate_visitor_data(client, visitor_type)
...
def generate_po_token(po_token_provider, identifier=''):
    ...
    if po_token_provider < 4:
        ...
        if settings.proxy_url.strip() and settings.use_proxy != 0:
